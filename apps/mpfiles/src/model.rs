...
/// first-frame thumbnail and an `mpvideo` association. The rest of
...
// There is no association table here: `mp_wm_api::viewer_for` is the one the
...
mod tests {
    ...
    fn reads_a_head_of_lines() {
        let dir = std::env::temp_dir().join("mpfiles-test-head");
